"""
YouTube Downloader — Multi Download
Just run:  python ytdown.py  or double-click START.bat
...
def find_tool(name):
    """
    Find yt-dlp or ffmpeg.
    When bundled with PyInstaller --add-binary, they are extracted to
    sys._MEIPASS (a temp folder). We check there first, then the exe
    folder, then PATH.
    """
    import shutil

    # 1. Check PyInstaller temp extraction folder (_MEIPASS)
    #    This is where --add-binary puts bundled files at runtime
    meipass = getattr(sys, "_MEIPASS", None)
    if meipass:
        for suffix in (".exe", ""):
            p = os.path.join(meipass, name + suffix)
            if os.path.exists(p):
                return p

    # 2. Check same folder as the EXE / script
    ...
    for suffix in (".exe", ""):
        p = os.path.join(exe_dir, name + suffix)
        if os.path.exists(p):
            return p
...
try:
    ...
    print("\n  Flask not found. Installing automatically...")
    install("flask")
    import flask
...
.url-entry-top{display:flex;gap-8px;align-items:center;gap:6px;min-width:0}
...
/* divider */
.divider{height:1px;background:var(--gray-100);margin:2px 0 10px}
...
<div class="app">
  ...
  <!-- URLs Card -->
  <div class="card">
    <div class="section-label">Video URLs <span id="urlCount" style="color:var(--brand);font-variant-numeric:tabular-nums">1</span></div>
...
def download():
    ...
    def generate():
        ...
        def run():
            if not YTDLP_BIN:
                q.put("__ERROR__yt-dlp.exe not found! Put yt-dlp.exe in the same folder as YT Downloader.exe")
                return
            ...
            # Timestamp clipping using --download-sections
            if start or end:
            ...
            try:
                ...
                q.put("__ERROR__yt-dlp not found! Run: pip install yt-dlp")
        ...
        while True:
            ...
            if line.startswith("__EXIT__"):
                ...
                yield f"data: {json.dumps({'type':'done','code':code,'dir':DOWNLOAD_DIR})}\n\n"
                break
